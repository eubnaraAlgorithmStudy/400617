...
int cache[1000+5][1000+5][1000+5];

// 어디서에서 이 자리에 오게 되었는지
int parent[1000+5][1000+5][1000+5][2];
...
void dp(int numOfIncidents) {
    // base case
    cache[1][0][1] = distance(incident[1], incident[0]);
    cache[0][1][1] = distance(incident[1], incident[w+1]);

    parent[1][0][0][0] = parent[1][0][0][1] =  1;
    parent[0][1][1][0] = parent[0][1][1][1] =  n;

    for(int i = 2; i <= numOfIncidents; i++) {
        // cop1이 현재 사건으로 가는 경우
        for(int j = 0; j < i; j++) {
            cache[i][j][i] = 987654321; // 초기화
        }
        // 바로 전 사건도 cop1이 처리한 경우
        for(int j = 0; j < i-1; j++) {
            cache[i][j][i] = cache[i-1][j][i-1] + distance(incident[i-1], incident[i]);
            // 바로 전에 어느 위치에서 왔나.
            parent[i][j][i][0] = i-1;
            parent[i][j][i][1] = j;
        }
        ...
        for(int j = 0; j < i-1; j++) {
            if (cache[i][i-1][i] > cache[j][i-1][i-1] + distance(incident[j], incident[i]) ) {
                cache[i][i-1][i] = cache[j][i-1][i-1] + distance(incident[j], incident[i]);
                parent[i][i-1][i][0] = j;
                parent[i][i-1][i][1] = i-1;
            }
        }
        ...
        for(int j = 0; j < i-1; j++) {
            cache[j][i][i] = cache[j][i-1][i-1] + distance(incident[i-1], incident[i]);
            // 바로 전에 어느 위치에서 왔나.
            parent[j][i][i][0] = j;
            parent[j][i][i][1] = i-1;
        }
        ...
        for(int j = 0; j < i-1; j++) {
            if (cache[i-1][i][i] > cache[i-1][j][i-1] + distance(incident[j], incident[i])) {
                cache[i-1][i][i] = cache[i-1][j][i-1] + distance(incident[j], incident[i]);
                parent[i-1][i][i][0] = i-1;
                parent[i-1][i][i][1] = j;
            }
        }
    }
...
void printOrder(int endCop1, int endCop2) {
  ...
  for(int i = w; i >= 1; i--) {
      ...
      endCop1 = parent[endCop1][endCop2][i][0];
      endCop2 = parent[endCop1][endCop2][i][1];
  }
  for(int i = w; i >= 1; i--) {
    printf("%d\n", order.back());
    order.pop_back();
